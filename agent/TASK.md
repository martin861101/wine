...
Use an actual search API/provider suitable for server-side use.

Inspect the existing project/secrets first to determine whether a search provider already exists.

If not, implement the search layer behind a provider abstraction so the provider can be replaced later without changing Gemini's `search_web` tool.
...
Do not tightly couple Gemini tool definitions to a specific search vendor.

If a new provider requires an API key, document the required Supabase secret clearly.
